...
Singleton {
    ...
    Process {
        ...
        command: [
            "/usr/bin/bash", "-c",
            "cat '" + root.repoPath + "/sdata/VERSION' 2>/dev/null || cat '" + root.repoPath + "/VERSION' 2>/dev/null || cat '" + root.configDir + "/sdata/VERSION' 2>/dev/null || cat '" + root.configDir + "/VERSION' 2>/dev/null || echo ''"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
                const ver = (text ?? "").trim()
