...
VERSION_FILE_REPO="${REPO_ROOT}/sdata/VERSION"
[[ ! -f "$VERSION_FILE_REPO" && -f "${REPO_ROOT}/VERSION" ]] && VERSION_FILE_REPO="${REPO_ROOT}/VERSION"
...
get_installed_version() {
    ...
    if [[ "$(get_installed_install_mode)" == "repo-link" ]]; then
        local runtime_dir
        runtime_dir="$(get_runtime_shell_dir)"
        if [[ -n "$runtime_dir" && -f "$runtime_dir/sdata/VERSION" ]]; then
            head -1 "$runtime_dir/sdata/VERSION" | tr -d '[:space:]'
            return
        elif [[ -n "$runtime_dir" && -f "$runtime_dir/VERSION" ]]; then
