...
if [[ -d "dots/.config/fish/conf.d" ]]; then
  v mkdir -p "${XDG_CONFIG_HOME}/fish/conf.d"
  install_dir "dots/.config/fish/conf.d" "${XDG_CONFIG_HOME}/fish/conf.d"
fi

if [[ -f "dots/.config/starship-ssh.toml" ]]; then
...
if [[ -f "scripts/niri-anim-switcher" ]]; then
  ...
  install_file "scripts/niri-anim-switcher" "${XDG_BIN_HOME}/niri-anim-switcher"
  chmod +x "${XDG_BIN_HOME}/niri-anim-switcher"
fi

# One-Click Ecosystem Updater
if [[ -f "scripts/update-all.sh" ]]; then
  v mkdir -p "${XDG_BIN_HOME}"
...
if [[ -f "scripts/install-devops-tools.sh" ]]; then
  bash "scripts/install-devops-tools.sh" || true
fi

# SSH fish bridge in ~/.bashrc
if [[ -f "${HOME}/.bashrc" ]] && ! grep -q "exec fish -l" "${HOME}/.bashrc" 2>/dev/null; then
  cat << 'EOF' >> "${HOME}/.bashrc"

# Auto-switch to fish for interactive SSH sessions
if [[ -n "$SSH_CONNECTION" || -n "$SSH_CLIENT" || -n "$SSH_TTY" ]] && [[ $- == *i* ]] && command -v fish >/dev/null 2>&1; then
    exec fish -l
fi
EOF
fi
...
if [[ -n "${II_TARGET}" && -d "${II_TARGET}" ]]; then
  ...
  if [[ -f "${REPO_ROOT}/sdata/VERSION" ]]; then
    REPO_VERSION=$(cat "${REPO_ROOT}/sdata/VERSION" | tr -d '[:space:]')
