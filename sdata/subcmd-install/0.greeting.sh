...
if [[ -f "${REPO_ROOT}/sdata/VERSION" ]]; then
    DETECTED_VERSION="v$(cat "${REPO_ROOT}/sdata/VERSION" | tr -d '[:space:]')"
elif [[ -f "${REPO_ROOT}/VERSION" ]]; then
    DETECTED_VERSION="v$(cat "${REPO_ROOT}/VERSION" | tr -d '[:space:]')"
elif [[ -f "sdata/VERSION" ]]; then
    DETECTED_VERSION="v$(cat "sdata/VERSION" | tr -d '[:space:]')"
elif [[ -f "VERSION" ]]; then
